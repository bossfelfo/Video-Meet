...
const participants = [
  { id: 1, name: "Melody Onyeocha", color: "bg-pink-400", company: "Tencent Holdings", pronouns: "(he/him)" },
  { id: 2, name: "Darrell Steward", color: "bg-green-400", company: "Microsoft" },
  { id: 3, name: "Bessie Cooper", color: "bg-blue-200", company: "Microsoft" },
  { id: 4, name: "Esther Howard", color: "bg-orange-400", company: "Dell Technologies" },
]
    ...
    <div className="flex flex-col h-screen bg-black text-white">
      ...
      <header className="flex items-center justify-between p-4">
        <div className="flex items-center space-x-4">
          ...
          <h1 className="text-lg font-medium">Navigating the path to success.</h1>
        ...
        {(showChat || showParticipants) && (
          <div className="w-80 bg-gray-900 rounded-lg ml-4 flex flex-col">
            <div className="p-2 border-b border-gray-800 flex">
              <Tabs defaultValue={showChat ? "chat" : "participants"} className="w-full">
                <TabsList className="w-full bg-gray-800">
                  <TabsTrigger value="chat" className="flex-1" onClick={() => setShowChat(true)}>
                    Chat
                  </TabsTrigger>
                  <TabsTrigger value="participants" className="flex-1" onClick={() => setShowParticipants(true)}>
                    Participants{" "}
                    <span className="ml-1 text-xs bg-gray-700 px-1.5 rounded-full">{participants.length}</span>
                  </TabsTrigger>
                </TabsList>
                <Button
                  variant="ghost"
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <div className="flex items-center">
                              ...
                              {p.pronouns && <span className="text-xs text-gray-400 ml-1">{p.pronouns}</span>}
                            </div>
                            <p className="text-xs text-gray-400">{p.company}</p>
                          </div>
                        </div>
                        <div className="flex items-center">
                          {currentSpeakerId === p.id && <div className="mr-2 text-xs text-green-400">Speaking</div>}
                          <MoreOptionsMenu
      ...
      <footer className="p-4 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          <Button
            variant="ghost"
            size="icon"
            className={`rounded-full ${audioEnabled ? "bg-gray-800 hover:bg-gray-700" : "bg-red-600 hover:bg-red-700"}`}
            onClick={toggleAudio}
          >
          ...
          <Button
            variant="ghost"
            size="icon"
            className={`rounded-full ${videoEnabled ? "bg-gray-800 hover:bg-gray-700" : "bg-red-600 hover:bg-red-700"}`}
            onClick={toggleVideo}
          >
          ...
          <Button
            variant="ghost"
            size="icon"
            className={`rounded-full ${isScreenSharing ? "bg-green-600 hover:bg-green-700" : "bg-gray-800 hover:bg-gray-700"}`}
            onClick={() => (isScreenSharing ? stopScreenShare() : setShowScreenShareModal(true))}
          >
            <PresentationScreen className="h-5 w-5" />
          </Button>

          <Button variant="ghost" size="icon" className="rounded-full bg-red-600 hover:bg-red-700" onClick={endCall}>
        ...
        <div className="flex items-center space-x-2">
          <Button
            variant="ghost"
            size="icon"
            className={`rounded-full ${showChat ? "bg-gray-700" : "bg-gray-800 hover:bg-gray-700"}`}
          ...
          <Button
            variant="ghost"
            size="icon"
            className={`rounded-full ${showParticipants ? "bg-gray-700" : "bg-gray-800 hover:bg-gray-700"}`}
      ...
      {/* Modals */}
      <AddUserDialog isOpen={showAddUserDialog} onClose={() => setShowAddUserDialog(false)} onAddUser={addUser} />
